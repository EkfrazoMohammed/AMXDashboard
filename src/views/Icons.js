...
function AddDrone() {
  ...
  //     newMaintenence = resultDateStr
  //     console.log(newMaintenence,"new data")

  //   } else if ( name === "time_in_service" || name === "Next_maintainance") {

  //     if (new Date(newMaintenence) < new Date(state.purchase_year)) {
  //       setState({...state,'Next_maintainance':newMaintenence})
  ...
  let handleChange = (e) => {
    ...
    if (name === "purchase_year") {
      ...
      inputDate.setFullYear(inputDate.getFullYear() + 1);
      const resultDateStr = inputDate.toISOString().slice(0, 10);
      const timeDifference = new Date(resultDateStr) - new Date();
      const daysDifference = Math.ceil(timeDifference / (1000 * 60 * 60 * 24));
      setState({
        ...state,
        [name]: value,
        mydays: resultDateStr,
        Next_maintainance: `${daysDifference} days`,
      });
    } else if (name === "time_in_service" || name === "Next_maintainance") {
      if (new Date(state.Next_maintainance) < new Date(state.purchase_year)) {
        setErrors({ ...errors, [name]: true });
      } else {
        setErrors({ ...errors, [name]: false });
      }
              ...
              <div className="popup-box-adddrone">
                ...
                <div className="wraper-dashboard">
                  <div className="container">
                    ...
                    <div class="form-group row" style={{display:'flex',justifyContent:'flex-start'}}>
                      <label for="staticEmail" class="col-form-label col-sm-3">
                        <span className="Add-drone-form-lable">
