...
function VideoAnalytics() {
  ...
  let folderIdo = localStorage.getItem("vfolder_id");
  ...
  const paramValue_project_id = queryParams.get("vproject_id");

  const paramValue_folder_id = queryParams.get("vfolder_id");
...
try {
  ...
  const fetchData = async () => {
    ...
    if (paramValue_project_id) {
      ...
      localStorage.setItem("vproject_id", paramValue_project_id);
    ...
    if (paramValue_folder_id) {
      ...
      localStorage.setItem("vfolder_id",paramValue_folder_id)
...
const folders11 = jsonData.filter((item) => {
  ...
  const reloadAndGetData = (item) => {
    console.log("=========...", item);
    history.push("/amx/vfolders?vfolder_id=" + item.name);
    localStorage.setItem("vfolder_id", item.name);
    window.location.reload();
  };
