...
function FolderFolders() {
  ...
  const handleUpload = async () => {
    ...
    const storageRefs = Object.entries(folderList).map(([folderName, files]) => {
      return files.map((file) =>
      console.log(file)
        // ref(storage, `/amx/${folderName}/${file.webkitRelativePath}`)
      );
    });

    const flatUploadTasks = storageRefs.flat();

    const uploadTasks = flatUploadTasks.map((storageRef, index) => {
      return uploadBytesResumable(storageRef, fileList.flat()[index]);
    });
      ...
      uploadTask.on(
        ...
        async () => {
          try {
            ...
            const data = {
              folders: Object.keys(fileList).map(
                (folderName) => folderName + "/"
              )[index], // Include folder structure
              name_folder: "test",
              folder_path: url,
            };
            ...
            if (response.ok) {
              ...
              if (completedUploads === flatUploadTasks.length) {
                await sendBytes(); // Call sendBytes function when all uploads are completed
                toast.success("New Folder and Files added!", {
                  position: "top-right",
                  autoClose: 5000,
                  hideProgressBar: false,
                  closeOnClick: true,
  ...
  const handleFolderUpload = (folder) => {
    // Process the selected folder and its contents here
    console.log('Uploaded folder:', folder);
    // Initiate the upload to your Django backend using an appropriate API
  };
      ...
      <BackgroundColorContext.Consumer>
        {({ color }) => (
          ...
          <div>Go Back</div>
                  ...
                  <FolderUpload handleFolderUpload={handleFolderChange} />
                        ...
                        <div className="wraper-dashboard">
                          <div className="wraper-card-content-dashboard text-center">
                            {/* <input type="file" onChange={handleChange} /> */}
                             ...
                             <DropFileInput  onFolderChange={handleFolderChange} />
